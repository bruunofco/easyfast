...
class Route extends RouteParent
{
    ...
    public function setConfigFile($file)
    {
        ...
        if ($handle) {
            ...
            while (!feof($handle)) {
                ...
                $verificaSo = explode("\r\n", $buffer);
                $lineSeparator = "\n";
                if(count($verificaSo) > 0) {
                    $lineSeparator = "\r\n";
                }

                $buffer = str_replace($lineSeparator, null, $buffer);
    ...
    public static function interceptRequests()
    {
        ...
        if (App::getAppConfig('routeAutomatic')) {
                ...
                if (is_array(self::$routeList)) {
                    foreach (self::$routeList as $p) {
                        ...
                        if ((isset($p['restful']) && $p['restful'] == 'true')) {
                            ...
                            $annCrossOrigin = AnnotationReader::getAnnotationMethod($class, $p['action'], 'EF_crossOrigin');
                            if ($annCrossOrigin) {
                                $rest->crossOrigin(true);
                            }
                    ...
                    $indexNotFound = self::getRoute(App::getAppConfig('defaultController'), 'notfound');
                    $httpStatus = isset($indexNotFound->routeConfig['httpStatus']) ? $indexNotFound->routeConfig['httpStatus'] : 200;
                    header("HTTP/1.1 {$httpStatus}");
                    Utils::callMethodArgsOrder("{$controllerDir}\\" . App::getAppConfig('defaultController'), $indexNotFound->routeConfig['action']);
                } else {
